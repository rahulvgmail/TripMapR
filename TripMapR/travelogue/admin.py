...
class TravelogueAdminForm(forms.ModelForm):
...
class TravelogueAdmin(admin.ModelAdmin):
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        ...
        return super(TravelogueAdmin, self).formfield_for_manytomany(db_field, request, **kwargs)

    def save_related(self, request, form, *args, **kwargs):
        """
        If the user has saved a travelogue with a photo that belongs only to
        different Sites - it might cause much confusion. So let them know.
        """
        super(TravelogueAdmin, self).save_related(request, form, *args, **kwargs)
        orphaned_photos = form.instance.orphaned_photos()
        if orphaned_photos:
            msg = ungettext(
                'The following photo does not belong to the same site(s)'
                ' as the travelogue, so will never be displayed: %(photo_list)s.',
    ...
    def add_to_current_site(modeladmin, request, queryset):
        ...
        current_site.travelogue_set.add(*queryset)
        msg = ungettext(
            "The travelogue has been successfully added to %(site)s",
    ...
    def remove_from_current_site(modeladmin, request, queryset):
        current_site = Site.objects.get_current()
        current_site.travelogue_set.remove(*queryset)
        msg = ungettext(
            "The travelogue has been successfully removed from %(site)s",
    ...
    def add_photos_to_current_site(modeladmin, request, queryset):
        ...
        msg = ungettext(
            'All photos in travelogue %(galleries)s have been successfully added to %(site)s',
            'All photos in galleries %(galleries)s have been successfully added to %(site)s',
            len(queryset)
        ) % {
            'site': current_site.name,
            'galleries': ", ".join(["'{0}'".format(travelogue.title)
                                    for travelogue in queryset])
        }
        messages.success(request, msg)
    ...
    def remove_photos_from_current_site(modeladmin, request, queryset):
        ...
        msg = ungettext(
            'All photos in travelogue %(galleries)s have been successfully removed from %(site)s',
...
admin.site.register(Travelogue, TravelogueAdmin)
