...
try:
    ...
    try:
        ...
        raise ImportError(
            'Travelogue was unable to import the Python Imaging Library. Please confirm it`s installed and available '
...
class Travelogue(models.Model):
    ...
    def sample(self, count=None, public=True):
        ...
        limited by the Travelogue_SAMPLE_SIZE setting.
        """
        if not count:
            count = SAMPLE_SIZE
    ...
    def photo_count(self, public=True):
        """Return a count of all the photos in this Travelogue."""
        if public:
    ...
    def public(self):
        """Return a queryset of all the public photos in this Travelogue."""
        return self.photos.is_public().filter(sites__id=settings.SITE_ID)

    def orphaned_photos(self):
        """
        Return all photos that belong to this Travelogue but don't share the
        Travelogue's site.
        """
    ...
    def title_slug(self):
        warnings.warn(
            DeprecationWarning("`title_slug` field in Travelogue is being renamed to `slug`. Update your code."))
        return self.slug
...
class BaseEffect(models.Model):
    ...
    def create_sample(self):
        try:
            ...
            raise IOError(
                'Travelogue was unable to open the sample image: %s.' % SAMPLE_IMAGE_PATH)
        im = self.process(im)
        buffer = BytesIO()
