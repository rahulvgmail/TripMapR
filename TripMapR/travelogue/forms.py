...
class UploadZipForm(forms.Form):
    ...
    title = forms.CharField(label=_('Title'),
                            ...
                            help_text=_('All uploaded photos will be given a title made up of this title + a '
                                        'sequential number.'))
    travelogue = forms.ModelChoiceField(Travelogue.objects.all(),
                                     label=_('Travelogue'),
                                     required=False,
                                     help_text=_('Select a travelogue to add these images to. Leave this empty to '
                                                 'create a new travelogue from the supplied title.'))
    caption = forms.CharField(label=_('Caption'),
                              required=False,
                              help_text=_('Caption will be added to all photos.'))
    description = forms.CharField(label=_('Description'),
                                  required=False,
                                  help_text=_('A description of this Travelogue.'))
    is_public = forms.BooleanField(label=_('Is public'),
                                   initial=True,
                                   required=False,
                                   help_text=_('Uncheck this to make the uploaded '
                                               'travelogue and included photographs private.'))
    ...
    def clean_title(self):
        ...
        if title and Travelogue.objects.filter(title=title).exists():
            raise forms.ValidationError(_('A travelogue with that title already exists.'))
        return title
    ...
    def clean(self):
        ...
        if not self['title'].errors:
            ...
            if not cleaned_data.get('title', None) and not cleaned_data['travelogue']:
                raise forms.ValidationError(
                    _('Select an existing travelogue, or enter a title for a new travelogue.'))
        return cleaned_data
    ...
    def save(self, request=None, zip_file=None):
        ...
        if self.cleaned_data['travelogue']:
            logger.debug('Using pre-existing travelogue.')
            travelogue = self.cleaned_data['travelogue']
        else:
            logger.debug(
                force_text('Creating new travelogue "{0}".').format(self.cleaned_data['title']))
            travelogue = Travelogue.objects.create(title=self.cleaned_data['title'],
                                             slug=slugify(self.cleaned_data['title']),
                                             description=self.cleaned_data['description'],
                                             is_public=self.cleaned_data['is_public'])
            travelogue.sites.add(current_site)
        for filename in sorted(zip.namelist()):
            ...
            title = ' '.join([travelogue.title, str(count)])
            slug = slugify(title)
        ...
        if request:
            messages.success(request,
                             _('The photos have been added to travelogue "{0}".').format(
                                 travelogue.title),
                             fail_silently=True)
